...
func TestGormQuerierDatabaseNameSQLite(t *testing.T) {
	q := newSQLiteGormQuerier(t)
	defer q.Close()
	// SELECT DATABASE() is MySQL-specific, SQLite will error — we test the error path
	_, err := q.DatabaseName()
	if err == nil {
		return
	}
}

func TestGormQuerierTableListSQLite(t *testing.T) {
	q := newSQLiteGormQuerier(t)
	defer q.Close()
	// SHOW TABLE STATUS is MySQL-specific, SQLite will error
	_, err := q.TableList()
	if err != nil {
		return
	}
...
func TestGormQuerierTableColumnsSQLite(t *testing.T) {
	q := newSQLiteGormQuerier(t)
	defer q.Close()
	// information_schema doesn't exist in SQLite
	_, err := q.TableColumns("testdb", "nonexistent")
	if err != nil {
		return
	}
}

func TestGormQuerierTableDDLSQLite(t *testing.T) {
	q := newSQLiteGormQuerier(t)
	defer q.Close()
	q.db.Exec("CREATE TABLE test_ddl (id INTEGER PRIMARY KEY)")
	// SHOW CREATE TABLE is MySQL-specific, SQLite will error
	_, err := q.TableDDL("test_ddl")
	if err != nil {
		return
	}
}
